...
partition_file = torch.load(f"train/FiniteAggregation_hash_mean_cifar_k{args.k}_d{args.d}.pth", weights_only=False, map_location=device)
...
def certify_batch_fa(k_poison, pred_classes, labels, per_datapoint_acc, shifts, relax=False):
    """ Solve MILP with Gurobi """
    model = gp.Model("Certification")
    
    gs = batch_certify_utils.compute_g_roe(f"'./certs/v_fa_roe_{args.evaluations}.pth'") # number of votes to flip before ensemble prediction flips for each test sample
    gss.append(gs[0]) ## DEBUG
...
def run_batch(from_idx, to_idx, shifts):
    ...
    fname = f"batch_certs/{str(args.evaluations)}/cert_accs_batch_{from_idx}_{to_idx}.pth"
    torch.save({
