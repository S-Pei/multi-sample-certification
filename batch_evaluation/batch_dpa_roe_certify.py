...
if not os.path.exists('./batch_certs/dpa_roe_' + str(args.evaluations)):
    os.makedirs('./batch_certs/dpa_roe_' + str(args.evaluations))
...
def run_batch(from_idx, to_idx):
    ...
    fname = f"batch_certs/dpa_roe_{str(args.evaluations)}/cert_accs_batch_{from_idx}_{to_idx}.pth"
    torch.save({
